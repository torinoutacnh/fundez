...
{
    public static class TigeDbContextFactoryHelper
    {
        /// <summary>
        ///     Setup DB Context based on have <param name="services"></param> data or not <br />
        ...
        public static void Build(IServiceCollection services, DbContextOptionsBuilder dbContextOptionsBuilder)
        {
            ...
            if (services != null)
            {
                services.AddDbContextPool<IDbContext, TigeDbContext>(
                    optionsBuilder => { Build(optionsBuilder, connectionString, commandTimeoutInSecond); },
                    dbContextPoolSize);
            }
            else
            {
            ...
            optionsBuilder
                .UseSqlServer(connectionString, sqlServerOptionsAction =>
                {
                    sqlServerOptionsAction
                        ...
                        .MigrationsAssembly(typeof(TigeDbContext).GetTypeInfo().Assembly.GetName().Name)
